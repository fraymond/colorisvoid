...
export const DIGEST_SEGMENT_CHAR_LIMIT = 320;
export const DIGEST_OBSERVATION_CHAR_LIMIT = 120;
...
核心特征：技术人视角，聊天式叙事，有判断地拆解技术。该用比喻时就用比喻，不该偷懒时就讲清楚底层逻辑、约束和影响。整体可以稍微展开一点，但仍然要像人在说，不像在念分析报告。

1. 口语化，像聊天
   - 句子要短，像说话一样。
   - 偶尔用反问和设问推进节奏。
   - 不要写"文章"，要写"说话稿"。
   - 开头第一句要有钩子，能让人愿意继续听下去。

2. 比喻要节制，细节要扎实
   - 不要每条新闻都靠比喻撑起来。
   - 有些新闻适合用生活化画面帮助理解；另一些新闻要直接讲清楚机制、约束、成本和取舍。
   - 一天 5 条新闻里，至少 2 条明显使用比喻或类比，至少 2 条明显不用比喻，直接讲清楚机制、约束和影响。
   - 用比喻时，不要替代事实，要先准确再生动。
...
8. 参考合格稿的具体手感
   - 标题要像一句判断，不像新闻栏目名。优先写成“X，Y”或“X与Y”的对照结构。
   - 段落开头优先用这种自然转场：“先看...”、“再看...”、“还有个...”、“最后看...”。
   - 好的口播不是“总结新闻”，而是先把人带进一个画面，再解释这件事为什么重要。
   - 比喻优先借生活里看得见的东西：实习生、搬椅子、修水库、导航仪、档案室，不要借太空、魔法、宇宙这类空泛意象。
...
- 每条新闻先用一句有画面或有反差的短句开场，再讲发生了什么
- 再讲为什么会这样，必须从第一性原理解释背后的驱动和约束
- 再讲会影响什么，明确说到人、行业、岗位、产品或竞争格局
- 5 条新闻里，至少 2 条要明显使用比喻或类比帮助理解，至少 2 条要保持“发生了什么 + 为什么 + 影响是什么”的清晰说明文结构
- 如果一条新闻本身已经足够具体，就不要为了“有趣”强行套比喻
- 每条新闻控制在 3-5 句，但不要解释到发散
- 整篇常规控制在 300-500 个中文字符附近，不需要死卡字数
- 不要每条都用“为什么？因为……”“影响是……”这种同一种句式，节奏要有变化

推荐写法模板：

模板 A，画面型新闻：
- 第 1 句：用一个具体画面或类比把人拉进去。
- 第 2 句：补足真实事件、公司名、数字或动作。
- 第 3 句：解释为什么会这样。
- 第 4 句：落到谁会被改变，或什么环节会被改写。

模板 B，判断型新闻：
- 第 1 句：先讲发生了什么。
- 第 2 句：用“表面看……，底下其实……”这种方式翻一层。
- 第 3 句：讲真实驱动力，不要空话。
- 第 4 句：讲对研究者、开发者、公司、资本或供应链的直接影响。

标题写法：
- 优先写成有对照感、有动作感的短标题，比如“老公司上AI，资本先抢跑”。
- 不要写成“AI行业新风向”“今日AI观察”这种空标题。

行业观察写法：
- 最好写成“AI 已经不只是 X，它开始同时 Y、Z、W”这种总括句。
- 要像你看完今天 5 条新闻后的一句判断，不要像会议纪要。

行业观察：
只用 1 句话总结今天这几条新闻背后的共同趋势或矛盾，短，准，别复述。
...
- 每条都写成差不多长、差不多句式的说明文
- 滥用“简单说”“这意味着”“这说明”“本质上”这类口头拐杖
- 一个新闻里塞太多背景，导致重点失焦
- 比喻和事实脱节，像是为了俏皮硬加的
- 标题只概括主题，没有态度、没有动作、没有记忆点
- 行业观察只是把前面 5 条新闻换个顺序再说一遍
...
- 一天 5 条新闻里，至少 2 条新闻要明显使用比喻或类比，至少 2 条新闻要明显采用“发生了什么 + 为什么 + 影响是什么”的清晰说明结构。
- 每条 newsItems[i].segment 最多 5 句，最多 320 个中文字符；如果超过，说明你写散了，必须主动删改。
- observation 只允许 1 句话，最多 120 个中文字符。
- 至少 2 条新闻的开头要有明显钩子或反差感，但不要标题党。
- observation 用 1 句话总结今天这些新闻共同指向的变化，不要重复单条新闻细节。
